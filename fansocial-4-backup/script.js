...
// filter tags on search tags input
document.addEventListener("DOMContentLoaded", function() {
        var tagCheckboxes = document.querySelectorAll('.ajax-filter-trigger-input');
        var searchInput = document.querySelector('.search-tags');

        searchInput.addEventListener("input", function() {
            var inputValue = this.value.toLowerCase();

            tagCheckboxes.forEach(function(checkbox) {
                var tagLabel = checkbox.parentElement.textContent.trim().toLowerCase();

                if (tagLabel.includes(inputValue)) {
...
sliderOne.addEventListener('change', updateSliderRange);
sliderTwo.addEventListener('change', updateSliderRange);
...
function updateSliderRange() {
    if (window.innerWidth >= offcanvas_size) {
        // Add a 2-second delay before calling getPosts()
        setTimeout(() => {
            getPosts();
...
function re_init_events() {
    ...
    document.querySelectorAll(".remove-filter").forEach((element) => {
        element.addEventListener("click", function () {
            ...
            if (target) {
                if (target.classList.contains("filter-age")) {
                    reset_range_slider();
...
function getPosts(parameters = {}) {
    ...
    // Filter breadcrumbs for age range
    const start = sliderOne.value;
    const end = sliderTwo.value;
    const text = `Age ${start} - ${end}`;
    const ageRangeId = "slider-1-" + sliderOne.value + "-slider-2-" + sliderTwo.value;
